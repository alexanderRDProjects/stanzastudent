...
$Gender = $_GET["Gender"];
$TargetCapacityMin = $_GET["TargetCapacityMin"]; //add to URL
$TargetCapacityMax = $_GET["TargetCapacityMax"]; //add to URL
$TargetSpacesMin = $_GET["TargetSpacesMin"];//add to URL
$TargetSpacesMax = $_GET["TargetSpacesMax"];//add to URL
...
foreach ($HouseData as &$HouseInst){
	...
	$counter = 0;
  $HouseSpaces = abs($HouseInst['Capacity'] - $HouseInst['FilledCapacity']);
  ...
  if ($TargetCapacity != NULL){
    $CurrentHouseScore += 3 * abs($TargetCapacity - $HouseInst['Capacity'])
  }
  $FilterCheck = false;
  if ($TargetPrice < $HouseInst['Price']){
    $FilterCheck = true;
  }
  if ($TargetCapacityMin > $HouseInst['Capacity'] or $TargetCapacityMax < $HouseInst['Capacity']){
    $FilterCheck = false;
  }
  if ($TargetSpacesMin > $HouseInst['Capacity'] or $TargetSpacesMax < $HouseInst['']){
    $FilterCheck = false;
  }
